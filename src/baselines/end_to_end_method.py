...
from src.dataset.PC_dataset import PCDataset, PCPatchDataset
...
def patch_with_vgg(train_excel, test_excel,
                   cuda_id, model, current_dir) -> dict:
    test = pd.read_excel(test_excel)
    test_result = {"center": test["center"],
                   "cancer": test["cancer"]}
    ...
    train_dataset = PCPatchDataset(
        excel_path=train_excel,
        transform=train_transform
    )
    test_dataset = PCPatchDataset(
